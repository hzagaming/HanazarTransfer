...
test("keeps the v1.1.0 release metadata and announcements aligned", async () => {
  ...
  const previous = await readFile(new URL("docs/announcements/v1.0.0.md", root), "utf8");
  ...
  assert.match(page, /styles\.css\?v=1\.1\.0/);
  assert.match(page, /app\.js\?v=1\.1\.0/);
  assert.match(page, /href="\.\/p2p\.html"/);
  assert.match(page, /meta name="referrer" content="no-referrer"/);
  assert.match(p2pPage, /V1\.1\.0/);
  assert.match(p2pPage, /styles\.css\?v=1\.1\.0/);
  assert.match(p2pPage, /p2p\.css\?v=1\.1\.0/);
  assert.match(p2pPage, /p2p\.js\?v=1\.1\.0/);
  ...
  assert.match(announcement, /v1\.1\.0/i);
  assert.match(previous, /v1\.0\.0/i);
  assert.match(history, /v1\.0\.0/i);
