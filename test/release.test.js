...
test("keeps the v0.2.2 release metadata and announcements aligned", async () => {
  ...
  const previous = await readFile(new URL("docs/announcements/v0.2.1.md", root), "utf8");
  ...
  assert.match(page, /styles\.css\?v=0\.2\.2/);
  assert.match(page, /app\.js\?v=0\.2\.2/);
  assert.match(page, /id="static-notice"/);
  assert.match(app, /hostname\.endsWith\("\.github\.io"\)/);
  assert.match(app, /addEventListener\("pagehide"/);
  assert.match(announcement, /v0\.2\.2/i);
  assert.match(previous, /v0\.2\.1/i);
  assert.match(history, /v0\.2\.1/i);
