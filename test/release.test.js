...
test("keeps the v0.3.1 release metadata and announcements aligned", async () => {
  ...
  const previous = await readFile(new URL("docs/announcements/v0.3.0.md", root), "utf8");
  ...
  assert.match(page, /V0\.3\.1/);
  assert.match(page, /styles\.css\?v=0\.3\.1/);
  assert.match(page, /app\.js\?v=0\.3\.1/);
  assert.match(page, /href="\.\/p2p\.html"/);
  assert.match(p2pPage, /V0\.3\.1/);
  assert.match(p2pPage, /p2p\.css\?v=0\.3\.1/);
  assert.match(p2pPage, /p2p\.js\?v=0\.3\.1/);
  assert.match(readme, /当前版本：`v0\.3\.1`/);
  assert.match(page, /id="static-notice"/);
  assert.match(app, /hostname\.endsWith\("\.github\.io"\)/);
  assert.match(app, /addEventListener\("pagehide"/);
  assert.match(announcement, /v0\.3\.1/i);
  assert.match(previous, /v0\.3\.0/i);
  assert.match(history, /v0\.3\.0/i);
